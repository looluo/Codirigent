...
//!
//! ```
//! use dirigent_session::DefaultBroadcastService;
...
use dirigent_core::{
    ...
    BroadcastVariables, DirigentEvent, SessionId,
};
...
impl DefaultBroadcastService {
    ...
    /// ```
    /// use dirigent_session::DefaultBroadcastService;
    /// use dirigent_core::BroadcastService;
    ///
    ...
    ///
    /// * `event_tx` - Event bus sender
    pub fn set_event_bus(&mut self, event_tx: broadcast::Sender<DirigentEvent>) {
    ...
    fn send_internal(
        ...
        self.emit(DirigentEvent::BroadcastSent {
            id,
        ...
        for status in &mut message.delivery_status {
            match self.deliver_to_session(status.session_id, content) {
                Ok(()) => {
                    ...
                    self.emit(DirigentEvent::BroadcastDelivered {
                        id,
                ...
                Err(e) => {
                    ...
                    self.emit(DirigentEvent::BroadcastDeliveryFailed {
                        id,
                        session_id: status.session_id,
                        error,
                    });
        ...
        self.emit(DirigentEvent::BroadcastComplete {
            id,
...
impl BroadcastService for DefaultBroadcastService {
    ...
    fn retry_failed(&mut self, id: BroadcastId) -> Result<()> {
        ...
        for session_id in events_to_emit {
            self.emit(DirigentEvent::BroadcastDelivered { id, session_id });
        }
...
mod tests {
    ...
    fn test_event_emission() {
        ...
        assert!(matches!(event, DirigentEvent::BroadcastSent { .. }));

        // Should receive BroadcastDelivered event
        let event = rx.try_recv().unwrap();
        assert!(matches!(event, DirigentEvent::BroadcastDelivered { .. }));

        // Should receive BroadcastComplete event
        let event = rx.try_recv().unwrap();
        assert!(matches!(event, DirigentEvent::BroadcastComplete { .. }));
    }
