...
//!
//! ```no_run
//! use dirigent_session::DefaultSkillManager;
...
use dirigent_core::skill::{Skill, SkillPreset, SkillType, TokenBudget};
use dirigent_core::traits::SkillManager as SkillManagerTrait;
...
pub struct DefaultSkillManager {
    ...
    event_tx: Option<broadcast::Sender<DirigentEvent>>,
}
...
impl DefaultSkillManager {
    ...
    /// use dirigent_core::events::DirigentEvent;
    ///
    /// let (tx, _rx) = broadcast::channel::<DirigentEvent>(16);
    /// let manager = DefaultSkillManager::with_event_bus(tx);
    /// ```
    pub fn with_event_bus(event_tx: broadcast::Sender<DirigentEvent>) -> Self {
        Self {
    ...
    fn emit(&self, event: DirigentEvent) {
...
impl SkillManagerTrait for DefaultSkillManager {
    ...
    fn enable_skill(&mut self, name: &str) -> Result<()> {
        ...
        self.emit(DirigentEvent::SkillEnabled {
            name: name.to_string(),
        });
    ...
    fn disable_skill(&mut self, name: &str) -> Result<()> {
        ...
        self.emit(DirigentEvent::SkillDisabled {
            name: name.to_string(),
        });
    ...
    fn recalculate_budget(&mut self) {
        ...
        if self.budget.is_exceeded() {
            ...
            self.emit(DirigentEvent::TokenBudgetExceeded {
                budget: self.budget,
            });
            ...
            self.emit(DirigentEvent::TokenBudgetWarning {
                budget: self.budget,
            });
        }
    }
    ...
    fn apply_preset(&mut self, preset_name: &str) -> Result<()> {
        ...
        self.emit(DirigentEvent::SkillPresetApplied {
    ...
    fn refresh(&mut self) -> Result<()> {
        ...
        self.emit(DirigentEvent::SkillsRefreshed {
...
mod tests {
    ...
    fn test_emit_skill_enabled_event() {
        let (tx, mut rx) = broadcast::channel::<DirigentEvent>(16);
        ...
        assert!(matches!(event, DirigentEvent::SkillEnabled { name } if name == "test"));
    }
    ...
    fn test_emit_skill_disabled_event() {
        ...
        // Check that event was emitted
        let event = rx.try_recv().unwrap();
        assert!(matches!(event, DirigentEvent::SkillDisabled { name } if name == "test"));
    }
    ...
    fn test_emit_preset_applied_event() {
        ...
        let event = rx.try_recv().unwrap();
        assert!(
            matches!(event, DirigentEvent::SkillPresetApplied { preset_name, .. } if preset_name == "minimal")
        );
    }
    ...
    fn test_emit_skills_refreshed_event() {
        ...
        // Check that event was emitted
        let event = rx.try_recv().unwrap();
        assert!(matches!(event, DirigentEvent::SkillsRefreshed { count: 0 }));
    }
    ...
    fn test_emit_token_budget_warning() {
        ...
        let event = rx.try_recv().unwrap();
        assert!(matches!(event, DirigentEvent::TokenBudgetWarning { budget } if budget.is_warning()));
    }
    ...
    fn test_emit_token_budget_exceeded() {
        ...
        let event = rx.try_recv().unwrap();
        assert!(
            matches!(event, DirigentEvent::TokenBudgetExceeded { budget } if budget.is_exceeded())
        );
    }
