...
//!
//! ```no_run
//! use dirigent_session::DefaultRalphLoopController;
//! use dirigent_core::{
...
use dirigent_core::{
    traits::RalphLoopController, DirigentEvent, EventBus, IterationResult, RalphLoopConfig,
    ...
    pub fn trigger_compaction(&self, session_id: SessionId) {
        if let Some((_, state)) = self.loops.get(&session_id) {
            if !state.status.is_finished() {
                ...
                self.emit(DirigentEvent::RalphLoopCompacted {
                    session_id,
    ...
    fn check_and_emit_stuck(&self, session_id: SessionId) {
        if let Some((config, state)) = self.loops.get(&session_id) {
                ...
                && !state.status.is_finished()
            {
                ...
                self.emit(DirigentEvent::RalphLoopStuck {
                    session_id,
...
impl RalphLoopController for DefaultRalphLoopController {
    fn start(&mut self, session_id: SessionId, config: RalphLoopConfig) -> Result<()> {
        ...
        self.emit(DirigentEvent::RalphLoopStarted {
            session_id,
            config: config.clone(),
        });
    ...
    fn pause(&mut self, session_id: SessionId) -> Result<()> {
        ...
        self.emit(DirigentEvent::RalphLoopPaused { session_id });
        Ok(())
    }
    ...
    fn resume(&mut self, session_id: SessionId) -> Result<()> {
        ...
        self.emit(DirigentEvent::RalphLoopResumed { session_id });
        Ok(())
    }
    ...
    fn stop(&mut self, session_id: SessionId) -> Result<()> {
        ...
        self.emit(DirigentEvent::RalphLoopCancelled { session_id });
        Ok(())
    }
    ...
    fn record_iteration(&mut self, session_id: SessionId, result: IterationResult) -> Result<()> {
        ...
        self.emit(DirigentEvent::RalphLoopIteration {
            session_id,
            iteration,
            passed,
            test_failures,
        ...
        if passed {
            ...
            self.emit(DirigentEvent::RalphLoopCompleted {
                session_id,
            ...
            self.emit(DirigentEvent::RalphLoopFailed {
                session_id,
...
mod tests {
    ...
    fn test_event_emission() {
        ...
        assert!(matches!(event, DirigentEvent::RalphLoopStarted { .. }));
    }
