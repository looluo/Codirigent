...
use dirigent_core::plugin::{Plugin, PluginManager, PluginState};
...
///
/// ```ignore
/// use dirigent_plugin::DefaultPluginManager;
...
mod tests {
    ...
    use dirigent_core::plugin::{PluginCapabilities, PluginMetadata};
    use std::any::Any;
    ...
    fn test_manager_plugins_dir() {
        let event_bus = Arc::new(MockEventBus::new());
        let manager = DefaultPluginManager::new(
            PathBuf::from("/home/user/.dirigent/plugins"),
            event_bus,
        );

        assert_eq!(manager.plugins_dir(), Path::new("/home/user/.dirigent/plugins"));
    }
