...
//!
//! ```
//! use dirigent_ui::clipboard_preview::ClipboardPreview;
//! use dirigent_ui::smart_clipboard::ThumbnailPreview;
//! use dirigent_ui::theme::DirigentTheme;
//! use std::path::PathBuf;
//!
//! // Create a preview component
//! let mut preview = ClipboardPreview::new(DirigentTheme::dark());
//!
//! // Show a preview
...
/// use dirigent_ui::theme::DirigentTheme;
///
/// let preview = ClipboardPreview::new(DirigentTheme::dark());
/// assert!(!preview.is_visible());
/// ```
#[derive(Debug, Clone)]
...
impl ClipboardPreview {
    ...
    /// ```
    pub fn new(theme: DirigentTheme) -> Self {
        Self {
            theme,
    ...
    /// # Returns
    ///
    /// A reference to the `DirigentTheme` used for rendering.
    ...
    /// ```
    /// use dirigent_ui::clipboard_preview::ClipboardPreview;
    /// use dirigent_core::{ImageData, ImageFormat};
    ///
    /// let image = ImageData {
    ///     bytes: vec![1, 2, 3, 4],
    ///     width: 1920,
...
mod tests {
    ...
    fn test_clipboard_preview_theme_methods() {
        let dark_theme = DirigentTheme::dark();
        let light_theme = DirigentTheme::light();
    ...
    fn test_clipboard_preview_clone() {
        let mut original = ClipboardPreview::new(DirigentTheme::dark());
        let thumbnail = ThumbnailPreview::new(
            vec![1, 2, 3],
            PathBuf::from("/tmp/test.png"),
            800,
