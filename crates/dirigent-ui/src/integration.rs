...
//!
//! ```no_run
//! use dirigent_ui::integration::DirigentIntegration;
//! use std::path::PathBuf;
//!
//! let integration = DirigentIntegration::new(PathBuf::from(".")).unwrap();
//!
//! // Create a session
...
use dirigent_core::{
    AppState, DefaultEventBus, DirigentEvent, EventBus, FileStorageService, ProcessMonitor,
...
/// Main integration layer connecting all Dirigent components.
///
...
pub struct DirigentIntegration {
    /// Session manager for PTY and session lifecycle.
...
impl DirigentIntegration {
    /// Create a new integration layer with default configuration.
    ///
    /// # Arguments
    ///
    /// * `project_dir` - The project directory where `.dirigent` will be created
    ///
    /// # Errors
    ///
    ...
    pub fn with_config(project_dir: PathBuf, config: IntegrationConfig) -> Result<Self> {
        info!(?project_dir, "Initializing Dirigent integration");
    ...
    fn handle_event(
        ...
        match event {
            DirigentEvent::SessionCreated { id } => {
                info!(%id, "Session created");
                if auto_save {
                    Self::save_state_internal(session_manager, storage);
                }
            }
            DirigentEvent::SessionClosed { id } => {
                info!(%id, "Session closed");
                if auto_save {
                    Self::save_state_internal(session_manager, storage);
                }
            }
            DirigentEvent::SessionStatusChanged { id, old, new } => {
                debug!(%id, ?old, ?new, "Session status changed");
            }
            DirigentEvent::InputRequired { session_id, pattern } => {
                info!(%session_id, ?pattern, "Input required");
            }
            DirigentEvent::InputProvided { session_id } => {
                debug!(%session_id, "Input provided");
            }
            DirigentEvent::SessionRenamed { id, old_name, new_name } => {
                debug!(%id, %old_name, %new_name, "Session renamed");
                if auto_save {
                    Self::save_state_internal(session_manager, storage);
                }
            }
            DirigentEvent::SessionGroupChanged { id, group, color } => {
    ...
    fn save_state_internal(
        ...
        if let Ok(manager) = session_manager.lock() {
            ...
            let state = AppState {
                sessions,
                layout: dirigent_core::LayoutMode::default(),
    ...
    ///
    /// Persists all session metadata to the `.dirigent/state.json` file.
    ...
    pub fn subscribe(&self) -> broadcast::Receiver<DirigentEvent> {
    ...
    pub fn publish(&self, event: DirigentEvent) {
...
mod tests {
    ...
    fn create_test_integration() -> (DirigentIntegration, TempDir) {
        ...
        let integration = DirigentIntegration::with_config(
    ...
    fn test_integration_subscribe() {
        ...
        integration.publish(DirigentEvent::SessionCreated { id: SessionId(1) });
    ...
    fn test_handle_event_session_created() {
        ...
        DirigentIntegration::handle_event(&event, &session_manager, &storage, false);
        // Should not panic
    }
    ...
    fn test_handle_event_session_closed() {
        ...
        let event = DirigentEvent::SessionClosed { id: SessionId(1) };
        DirigentIntegration::handle_event(&event, &session_manager, &storage, false);
        // Should not panic
    }
    ...
    fn test_handle_event_input_provided() {
        ...
        let event = DirigentEvent::InputProvided { session_id: SessionId(1) };
        DirigentIntegration::handle_event(&event, &session_manager, &storage, false);
        // Should not panic
    }
