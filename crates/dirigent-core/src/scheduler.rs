...
///
/// ```
/// use dirigent_core::SchedulerMode;
///
...
///
/// ```
/// use dirigent_core::{SchedulerConfig, SchedulerMode};
///
...
impl TaskQueue {
    ...
    ///
    /// ```
    /// use dirigent_core::{TaskQueue, SchedulerConfig, DefaultEventBus};
    /// use std::sync::Arc;
    ///
    /// let event_bus = Arc::new(DefaultEventBus::new(16));
    ...
    ///
    /// ```
    /// use dirigent_core::{TaskQueue, SchedulerConfig, QueueState, Task, DefaultEventBus};
    /// use std::sync::Arc;
    ///
    /// let event_bus = Arc::new(DefaultEventBus::new(16));
    /// let mut queue = TaskQueue::new(SchedulerConfig::default(), event_bus);
    ...
    ///
    /// ```
    /// use dirigent_core::{TaskQueue, SchedulerConfig, SchedulerMode, DefaultEventBus};
    /// use std::sync::Arc;
    ///
    /// let event_bus = Arc::new(DefaultEventBus::new(16));
    /// let config = SchedulerConfig {
    ...
    ///
    /// ```
    /// use dirigent_core::{TaskQueue, SchedulerConfig, Task, TaskId, DefaultEventBus};
    /// use std::sync::Arc;
    ///
    /// let event_bus = Arc::new(DefaultEventBus::new(16));
    /// let mut queue = TaskQueue::new(SchedulerConfig::default(), event_bus);
    ...
    pub fn enqueue(&mut self, task: Task) -> Result<()> {
        ...
        // Publish event
        self.event_bus.publish(DirigentEvent::TaskCreated { id });
    ...
    pub fn assign_task(&mut self, task_id: &TaskId, session_id: SessionId) -> Result<()> {
        ...
        // Publish event
        self.event_bus.publish(DirigentEvent::TaskAssigned {
    ...
    pub fn complete_task(&mut self, task_id: &TaskId, success: bool) -> Result<()> {
        ...
        // Publish event
        self.event_bus.publish(DirigentEvent::TaskCompleted {
            task_id: task_id.clone(),
            success,
        });
