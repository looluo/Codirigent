...
///
/// ```
/// use dirigent_core::persistence_service::AutoSaveConfig;
///
...
///
/// Uses the `.dirigent` directory in the project for storage.
...
///
/// ```no_run
/// use dirigent_core::persistence_service::{DefaultPersistenceService, PersistenceService};
/// use dirigent_core::persistence::PersistentState;
/// use std::path::Path;
///
...
impl DefaultPersistenceService {
    ...
    ///
    /// ```
    /// use dirigent_core::persistence_service::DefaultPersistenceService;
    /// use std::path::Path;
    ///
    /// let service = DefaultPersistenceService::new(Path::new("/project"));
    /// ```
    pub fn new(project_dir: &Path) -> Self {
        Self {
            dirigent_dir: project_dir.join(".dirigent"),
        }
    }
    ...
    fn checkpoints_path(&self) -> PathBuf {
        self.dirigent_dir.join("checkpoints")
    }
...
mod tests {
    ...
    fn test_persistence_service_new() {
        ...
        assert_eq!(
            service.dirigent_dir(),
            temp.path().join(".dirigent").as_path()
        );
    }
    ...
    fn test_state_file_path() {
        ...
        assert_eq!(path, temp.path().join(".dirigent").join("state.json"));
    }
    ...
    fn test_checkpoints_dir() {
        ...
        assert_eq!(path, temp.path().join(".dirigent").join("checkpoints"));
    }
