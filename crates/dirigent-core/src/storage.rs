...
//! ## Directory Structure
//!
//! All data is stored in the `.dirigent` directory:
//!
//! ```text
//! .dirigent/
...
///
/// Stores all data in the `.dirigent` directory as JSON files
/// for portability and debuggability.
///
/// # Example
///
/// ```no_run
/// use dirigent_core::storage::FileStorageService;
/// use dirigent_core::traits::StorageService;
/// use std::path::Path;
///
...
pub struct FileStorageService {
    /// Path to the .dirigent directory.
    dirigent_dir: PathBuf,
}

impl FileStorageService {
    /// Create a new storage service for the given project directory.
    ///
    /// Creates the `.dirigent` directory and its subdirectories if they don't exist.
    ///
    /// # Arguments
    ///
    /// * `project_dir` - The project root directory where `.dirigent` will be created.
    ///
    /// # Errors
    ///
    ...
    pub fn new(project_dir: &Path) -> Result<Self> {
        let dirigent_dir = project_dir.join(".dirigent");
        debug!("Creating storage service at {}", dirigent_dir.display());
        let service = Self { dirigent_dir };
        service.ensure_directories()?;
        Ok(service)
    }

    /// Create a storage service from an existing `.dirigent` directory.
    ///
    /// This is useful when you already have the `.dirigent` path directly.
    ///
    /// # Arguments
    ///
    /// * `dirigent_dir` - The path to the `.dirigent` directory.
    ///
    /// # Errors
    ///
    /// Returns an error if the directories cannot be created.
    ...
    ///
    /// let storage = FileStorageService::from_dirigent_dir(
    ///     PathBuf::from("/path/to/project/.dirigent")
    /// ).unwrap();
    /// ```
    #[instrument(skip_all, fields(dirigent_dir = %dirigent_dir.display()))]
    pub fn from_dirigent_dir(dirigent_dir: PathBuf) -> Result<Self> {
        debug!(
            "Creating storage service from existing dir: {}",
            dirigent_dir.display()
        );
        let service = Self { dirigent_dir };
        service.ensure_directories()?;
        Ok(service)
    ...
    ///
    /// Creates the `.dirigent` directory and the `tasks` subdirectory
    /// if they don't already exist. This operation is idempotent.
    fn ensure_directories(&self) -> Result<()> {
        fs::create_dir_all(&self.dirigent_dir)
            .with_context(|| format!("Failed to create .dirigent directory at {:?}", self.dirigent_dir))?;
        fs::create_dir_all(self.tasks_dir())
            .with_context(|| format!("Failed to create tasks directory at {:?}", self.tasks_dir()))?;
        debug!("Directories ensured at {}", self.dirigent_dir.display());
        Ok(())
    }

    /// Get the path to `state.json`.
    fn state_path(&self) -> PathBuf {
        self.dirigent_dir.join("state.json")
    }

    /// Get the path to `config.json`.
    #[allow(dead_code)]
    fn config_path(&self) -> PathBuf {
        self.dirigent_dir.join("config.json")
    }

    /// Get the path to the `tasks` directory.
    fn tasks_dir(&self) -> PathBuf {
        self.dirigent_dir.join("tasks")
    }
...
mod tests {
    ...
    fn test_new_creates_directories() {
        ...
        assert!(storage.dirigent_dir.exists());
    ...
    fn test_new_creates_dirigent_subdirectory() {
        ...
        let expected = temp.path().join(".dirigent");
        assert_eq!(storage.dirigent_dir, expected);
    }

    #[test]
    fn test_from_dirigent_dir() {
        let temp = TempDir::new().unwrap();
        let dirigent_path = temp.path().join(".dirigent");
        let storage = FileStorageService::from_dirigent_dir(dirigent_path.clone()).unwrap();

        assert_eq!(storage.dirigent_dir, dirigent_path);
        assert!(storage.dirigent_dir.exists());
        assert!(storage.tasks_dir().exists());
    }
    ...
    #[test]
    fn test_dirigent_dir_accessor() {
        let temp = TempDir::new().unwrap();
        let storage = FileStorageService::new(temp.path()).unwrap();

        let expected = temp.path().join(".dirigent");
        assert_eq!(storage.dirigent_dir(), expected.as_path());
    }
    ...
    fn test_state_path() {
        ...
        let expected = temp.path().join(".dirigent").join("state.json");
    ...
    fn test_config_path() {
        ...
        let expected = temp.path().join(".dirigent").join("config.json");
    ...
    fn test_tasks_dir() {
        ...
        let expected = temp.path().join(".dirigent").join("tasks");
    ...
    fn test_task_path() {
        ...
        let expected = temp.path().join(".dirigent").join("tasks").join("task-001.json");
    ...
    fn test_atomic_write() {
        ...
        let path = storage.dirigent_dir.join("test.json");
