...
///
/// ```ignore
/// use dirigent_core::{EventBus, DirigentEvent, SessionId};
///
...
///
/// Handles reading/writing to the `.dirigent` directory.
...
pub trait StorageService: Send + Sync {
    ...
    ///
    /// Returns the path to the project's `.dirigent` directory.
...
mod tests {
    ...
    impl EventBus for MockEventBus {
        ...
        fn publish(&self, _event: DirigentEvent) {}
    }
    ...
    fn test_mock_storage_service() {
        let storage = MockStorageService {
            dir: std::path::PathBuf::from("/tmp/.dirigent"),
        };

        assert_eq!(
            storage.dirigent_dir(),
            std::path::Path::new("/tmp/.dirigent")
        );
