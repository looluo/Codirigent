...
fn test_state_with_layout_modes() {
    ...
    state.layout = LayoutMode::Custom {
        positions: vec![
            (SessionId(1), dirigent_core::GridPosition { row: 0, col: 0 }),
            (SessionId(2), dirigent_core::GridPosition { row: 0, col: 1 }),
        ],
    };
    storage.save_state(&state).unwrap();
    let loaded = storage.load_state().unwrap();
...
#[test]
fn test_from_dirigent_dir_constructor() {
    let temp = TempDir::new().unwrap();
    ...
    // Then use from_dirigent_dir
    let dirigent_path = temp.path().join(".dirigent");
    let storage2 = FileStorageService::from_dirigent_dir(dirigent_path).unwrap();
    let loaded = storage2.load_state().unwrap();
    assert_eq!(loaded.sessions.len(), 1);
}
...
fn test_storage_directory_structure() {
    ...
    let dirigent_dir = temp.path().join(".dirigent");
    assert!(dirigent_dir.exists());
    assert!(dirigent_dir.join("state.json").exists());
    assert!(dirigent_dir.join("tasks").exists());
    assert!(dirigent_dir.join("tasks").join("task-001.json").exists());
}
