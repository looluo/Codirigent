...
use codirigent_core::{SessionStatus, CodirigentEvent, SessionManager, EventBus};
...
async fn test_session_complete_lifecycle() {
    ...
    // Test: Create session
    let session_id = fixture.create_session("test-session")
    ...
    assert!(event.is_ok() && event.unwrap().is_some(), "SessionCreated event not received");
    ...
    // Test: Close session
    fixture.session_manager.lock().unwrap()
    ...
    assert!(event.is_ok() && event.unwrap().is_some(), "SessionClosed event not received");
}
...
async fn test_multiple_sessions_isolated() {
    ...
    let session1 = fixture.create_session("session-1")
        .expect("Failed to create session 1");
    let session2 = fixture.create_session("session-2")
        .expect("Failed to create session 2");
    let session3 = fixture.create_session("session-3")
